/* ------------------------------------------------------------
 This is the file "laplaceoclbem3d.c" of the H2Lib package.
 All rights reserved, Sven Christophersen 2015
 ------------------------------------------------------------ */

/**
 * @file laplaceoclbem3d.c
...
static void
fill_cc_ocl_gpu_wrapper_laplacebem3d(void *data, uint kernel)
{
...
static void
fill_slp_cc_dist_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 0);
}

static void
fill_slp_cc_vert_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 1);
}

static void
fill_slp_cc_edge_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 2);
}

static void
fill_slp_cc_iden_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 3);
}

static void
fill_slp_cc_sing_cpu_wrapper_laplacebem3d(void *data)
{
  merge_data_nf *mdata = (merge_data_nf *) data;
  ...
  for (t = 0; t < pos; ++t) {
	...
	- (A_s[2] * Ay + B_s[2] * By + C_s[2] * Cy);
      ...
      sum += wq[q] * REAL_RSQRT(dx * dx + dy * dy + dz * dz);
    }
    *(addr[t]) = sum * factor;
  }
}
...
void
fill_slp_cc_near_task_laplacebem3d(const uint * ridx, const uint * cidx,
				   pcbem3d bem, bool ntrans, pamatrix N)
{
  ...
  if (nf == NULL) {
    op_cb.dist = fill_slp_cc_dist_gpu_wrapper_laplacebem3d;
    op_cb.vert = fill_slp_cc_vert_gpu_wrapper_laplacebem3d;
    op_cb.edge = fill_slp_cc_edge_gpu_wrapper_laplacebem3d;
    op_cb.iden = fill_slp_cc_iden_gpu_wrapper_laplacebem3d;
    op_cb.sing_cpu = fill_slp_cc_sing_cpu_wrapper_laplacebem3d;
...
void
fill_slp_cc_far_task_laplacebem3d(const uint * ridx, const uint * cidx,
				  pcbem3d bem, bool ntrans, pamatrix N)
{
  nearfield_args *nf_args;
...
static void
fill_dlp_cc_dist_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 4);
}

static void
fill_dlp_cc_vert_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 5);
}

static void
fill_dlp_cc_edge_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 6);
}

static void
fill_dlp_cc_iden_gpu_wrapper_laplacebem3d(void *data)
{
  fill_cc_ocl_gpu_wrapper_laplacebem3d(data, 7);
}

static void
fill_dlp_cc_sing_cpu_wrapper_laplacebem3d(void *data)
{
  merge_data_nf *mdata = (merge_data_nf *) data;
  pcbem3d   bem = mdata->bem;
  const pcsurface3d gr = bem->gr;
  ...
  for (t = 0; t < pos; ++t) {
	...
	sum += wq[q] * (dx * ns[0] + dy * ns[1] + dz * ns[2])
	  / (norm * REAL_SQRT(norm));
      }
      *(addr[t]) = sum * factor;
    }
...
void
fill_dlp_cc_near_task_laplacebem3d(const uint * ridx, const uint * cidx,
				   pcbem3d bem, bool ntrans, pamatrix N)
{
  nearfield_args *nf_args;
  ...
  if (nf == NULL) {
    op_cb.dist = fill_dlp_cc_dist_gpu_wrapper_laplacebem3d;
    op_cb.vert = fill_dlp_cc_vert_gpu_wrapper_laplacebem3d;
    op_cb.edge = fill_dlp_cc_edge_gpu_wrapper_laplacebem3d;
    op_cb.iden = fill_dlp_cc_iden_gpu_wrapper_laplacebem3d;
    op_cb.sing_cpu = fill_dlp_cc_sing_cpu_wrapper_laplacebem3d;
...
void
fill_dlp_cc_far_task_laplacebem3d(const uint * ridx, const uint * cidx,
				  pcbem3d bem, bool ntrans, pamatrix N)
{
  nearfield_args *nf_args;
...
static void
init_laplacebem3d_opencl(pcbem3d bem)
{
  uint      num_kernels;
  ...
  for (i = 0; i < num_devices; ++i) {
    ocl_bem3d.mem_q_xw[i] = clCreateBuffer(ocl_system.contexts[i], mem_rflags,
  ...
  for (i = 0; i < num_devices; ++i) {
    ocl_bem3d.mem_q2_xw[i] =
      clCreateBuffer(ocl_system.contexts[i], mem_rflags, 2 * q * sizeof(real),
		     NULL, &res);
    CL_CHECK(res)
      res = clEnqueueWriteBuffer(ocl_system.queues[i * num_queues],
  ...
  for (i = 0; i < num_devices; ++i) {
    ocl_bem3d.mem_gr_x[i] = clCreateBuffer(ocl_system.contexts[i], mem_rflags,
					   3 * v * sizeof(real), NULL, &res);
    CL_CHECK(res)

      ocl_bem3d.mem_gr_t[i] =
      clCreateBuffer(ocl_system.contexts[i], mem_rflags, 3 * t * sizeof(uint),
		     NULL, &res);
    CL_CHECK(res)
...
void
del_laplace_ocl_bem3d(pbem3d bem)
{
